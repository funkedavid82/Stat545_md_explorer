...
c("hi", 5)
letters
letters[7] #subsetting vectors with square bracket
(1:10) ^ 2
```

```{r}
(1:10) * (1:2) #vector 1 to 10 multiplied by vector 1 and 2.
```
...
```{r}
str(wesanderson)
x <- character(0) #initiate a character vector of length 0
...
```{r}
str(got_chars, list.len = 4) #only showing the first 4
str(got_chars, list.len = 4, max.level = 2) #how many component of list do you want to see
```
...
```{r}
map_dbl(x, sqrt) #an output vector of type double
map_chr(x, sqrt)
...
```{r}
map_chr(x, str_c, "potato.", sep="-") #apply potato to the vector x
```
...
#Method2 : head
map_chr(wesanderson, head, n=1) #subsetting and taking the first element of the data frame
...
map_chr(wesanderson, 1) %>% unname() #if you don't want the names there
```
...
#{} is used to pipe into more than one function at a time.
```
