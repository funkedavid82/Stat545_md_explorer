...
```{r}
str_split(fox, pattern = " ") #splits the sentence fox

str_split(fox, pattern = " ")[[1]] %>% length() #return the number of words in the sentence fox
...
```{r}
fox %>% 
  str_split("") %>% # "" without space is different from " " with space
  `[[`(1) %>%
  str_to_lower() %>% #convert all to lower case
  table() #put it in table function
...
fruit %>% head
fruit %>% 
    str_pad(width=7, side="right", pad="$") %>% #make all character 7 by adding $ to the ones less than 7.
    head()
```
...
```{r}
str_length(fruit) #number of characters in each words of fruits
length(fruit) #there are 80 fruits specified
```
...
words[1:4]
words[5:8]
str_c(words[1:4], words[5:8], sep=" & ") #bind two words one from words[1:4] and the other from words[5:8] with &
str_c(words[3:4], words[5:8], sep=" & ")
str_c(words[3:4], words[5:8], sep=" & ", collapse=", ") #removes space and separate by comma
```
...
```{r}
(num <- str_pad(1:12, width=2, side = "left", pad = "0")) #make exactly two chracters and add 0 to the left of anyone less than 2
```
...
```{r}
str_subset(countries, pattern = "i.a") #returns all countries with i and something a i.e i.a
```
...
str_view_all(countries, pattern = "i.a", match=TRUE) #only returns entries of the vector that have a match with the specified pattren
```
...
```{r}
str_view_all_match(countries, pattern = "[^aeiou]") #bring out all single letters except "aeiou"
str_view_all_match(countries, pattern = "[^aeiou][^aeiou][^aeiou]") #bring out all three letters except "aeiou"
str_view_all_match(countries, pattern = "[^aeiou ][^aeiou ][^aeiou ]") #bring out all letters except "aeiou"
...
```{r}
c("bear", "beer", "bar") %>% 
    str_view_all_match(pattern = "be(e|a)r") #bring out only words with ea after be and before r
```
...
```{r}
str_view_all_match(countries, "ro+") #countries after r and followed by o
```
...
```{r}
str_view_all_match(countries, "e{2}") #countries with e twice
```
...
- `^` corresponds to the __beginning__ of the line. But `[^]` means `not`
...
```{r}
str_view_all_match(countries, pattern = "^Ca") #countries that starts with ca
```

Countries without a vowel? The word should start with a non-vowel, continue as a non-vowel, and end:

```{r}
str_view_all_match(countries, "^[^aeiouAEIOU]*$") #countries without a vowel, and then repeat it. There is no country
```
